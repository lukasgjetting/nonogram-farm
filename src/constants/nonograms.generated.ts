export const NonogramSources = {
  "seeds.tomato.tomato": {
    "name": "Tomato",
    "content": "AAAAAaaA\nAAAaabAA\nAcabadbA\nceaBBdbf\nceBBBBBf\nccBBBBBf\nccBBBBff\nAccBBBfA",
    "colors": {"A":"#FEFFCE","B":"#B2322D","a":"#5A8D36","b":"#336311","c":"#E4433D","d":"#284811","e":"#FBB1A1","f":"#83221E"}
  },
  "seeds.tomato.tomato-slice": {
    "name": "Tomato Slice",
    "content": "AAaaaaaaAA\nAabbbbbbaA\nabBBBbBBba\nabBCCbcBba\nabbbbbcBba\nabBcbbbbba\nabBcbCCBba\nabBBbBBBba\nAabbbbbbaA\nAAaaaaaaAA",
    "colors": {"A":"#85B291","B":"#83221E","C":"#FBB1A1","a":"#B2322D","b":"#E4433D","c":"#FFC3B6"}
  },
  "seeds.tomato.caprese": {
    "name": "Caprese",
    "content": "aaaaaaAAAAAA\nabbbbAcdcdcd\nbBBbAccecdce\nbBBbAdfCCCfC\nbDDbAccdedcd\nbbbbAAcdcfcd\nEbbbbAAAAAAA\nFEbGbbAAAAAA\nFEbGbbbbbbbb\nFEbGbbEEbHHb\nFEbGbbaabaab\nEbbGbbaabaab",
    "colors": {"A":"#735D3B","B":"#E9FFFF","C":"#B1A38D","D":"#BBE2E2","E":"#E5E5E5","F":"#CFCFCF","G":"#B3B3B3","H":"#585858","a":"#A1865D","b":"#C3A77D","c":"#B2322D","d":"#FFFFFF","e":"#336311","f":"#284811"}
  },
  "seeds.pineapple.pineapple": {
    "name": "Pineapple",
    "content": "AAaabAbbbb\nAAbBabaaaa\nAbaaabaBBA\nbaBBaBBcAA\nbaBdaBedcA\nbaBefafecA\nAbdedeCedc\nAcefefeCec\nAcededeCec\nAcfefefefc",
    "colors": {"a":"#8AAC71","b":"#AED293","c":"#DAB95D","d":"#FFF59A","e":"#FFE382","f":"#FFDC62","A":"#EAFBFF","B":"#5E8C3E","C":"#FFFFFF"}
  },
  "seeds.pineapple.pineapple-slice": {
    "name": "Pineapple Slice",
    "content": "AaabAAAA\nddabAAbb\nBBddAbbc\nBBBedbcc\nBBeeedcA\nBeeefdAA\nBeefffdA\nBfffffdA",
    "colors": {"A":"#F3E3FF","B":"#FFF59A","a":"#AED293","b":"#8AAC71","c":"#5E8C3E","d":"#DAB95D","e":"#FFE382","f":"#FFDC62"}
  },
  "seeds.pineapple.pina-colada": {
    "name": "Pina Colada",
    "content": "aaAAaaAbcAAA\naaaAAAAABcAA\nAAdAAeeBebAA\nfdggggeBeggg\nfdhhCCeieCCC\nClChCeiiieCC\nClCCCejjjeCC\nClCCCekjkeCC\nCCCCmmeeemmm\nCCmmDDDeDDDD\nCmmDDDeeennn\nCmDDDDDnnnnn",
    "colors": {"A":"#BCEBFF","B":"#E4E4E4","C":"#FBFFC9","D":"#C3A77D","a":"#D5F2FF","b":"#DE5D8B","c":"#EC749F","d":"#8AAC71","e":"#E7FFFE","f":"#AED293","g":"#74B6F2","h":"#5E8C3E","i":"#FFF59A","j":"#FFDC62","k":"#DAB95D","l":"#735233","m":"#A1865D","n":"#A69A88"}
  },
  "seeds.lettuce.lettuce": {
    "name": "Lettuce",
    "content": "AAAaaaAA\nAAabbbaA\nAabcBcba\nAaccBcba\nAccBccba\nddBccaaA\ndBddaAAA\ndddAAAAA",
    "colors": {"A":"#DDA7FF","a":"#739B55","b":"#93C36E","c":"#B5F881","d":"#D0FBAF"}
  },
  "seeds.lettuce.garden": {
    "name": "Garden",
    "content": "aaaaAAAaaa\naaaAbbbAaa\naaAbAccAaa\naaAcAcAbAa\naaAcAAccAa\naaAcAccAaa\nBBdceCcBBd\nBdBAeCeBdB\nBdBBeeeBdB\nBdBBBBBBdB",
    "colors": {"A":"#739B55","B":"#9F7E60","C":"#F0FFE4","a":"#E4F4FF","b":"#93C36E","c":"#B5F881","d":"#7F5D3D","e":"#D0FBAF"}
  },
  "seeds.lettuce.cheeseburger": {
    "name": "Cheeseburger",
    "content": "AAaaaaaaaaAA\nAaaaaaaaaaaA\naaaaaaaaaaaa\naaBBBBBBBBaa\nABBbbbbbbBBA\nAAbbbbbbbbAA\nACCCCCCeeCCA\nAAcccCccccAA\naaccccccccaa\naaaaaaaaaaaa\nDaaaaaaaaaaD\nDDddddddddDD",
    "colors": {"A":"#404040","B":"#93C36E","C":"#FFDD66","D":"#DCDCDC","a":"#EDC892","b":"#B2322D","c":"#5F4025","d":"#C9C9C9","e":"#DDB62B"}
  },
  "seeds.carrot.carrot": {
    "name": "Carrot",
    "content": "AAAAdedA\nAAAaefed\nAAabcfed\nAAabcAfe\nAabcAAfe\nAabcAAfe\nabcAAAfA\nbcAAAAAA",
    "colors": {"A":"#FFD1F2","a":"#FFAE36","b":"#FB9600","c":"#C17400","d":"#85C959","e":"#4B8C1D","f":"#376816"}
  },
  "seeds.carrot.carrot-field": {
    "name": "Carrot Field",
    "content": "bbbaaGGGbb\nGGbbbbbbbG\nGGGGbbbGGG\nFbbFbbFbFb\nCcdCccCdCc\ncdcccEcccd\ncdEccBcccE\ncdBccdcccB\nDdccDdcDcd\nAdccAdcAcd",
    "colors": {"A":"#FFAE36","B":"#FB9600","C":"#C17400","D":"#85C959","E":"#4B8C1D","F":"#4B8C1D","G":"#FAFDFF","a":"#FFF59A","b":"#E4F4FF","c":"#9F7E60","d":"#7F5D3D"}
  },
  "seeds.carrot.bunny": {
    "name": "Bunny",
    "content": "AAAAAAAaaAAA\nAAAAaaaabAAA\nAAAacaabbAAA\nAAAdaaaAAAAA\nAeAAfaaaAAAA\nAghiAaaaaaaA\njgklaafaaaaa\njmkljAfaaaaa\njmmmjAAffaaa\njmmmjAAAaaaa\njmmjAAAAaaaA\nAjjjAAAaaaAA",
    "colors": {"A":"#C7FFBE","a":"#FFFFFF","b":"#FFE9F9","c":"#000000","d":"#FFD1F2","e":"#85C959","f":"#F2ECEC","g":"#FFAE36","h":"#4B8C1D","i":"#376816","j":"#7F5D3D","k":"#FB9600","l":"#C17400","m":"#9F7E60"}
  },
  "intro.trees": {
    "name": "Trees",
    "content": "AAAababbcc\nAAababbbbc\nAAabbabdbc\nAAAaaeadce\nAffaaeadae\nfghiaeadae\nBBBBBBBkkk\nBBBBBkkkkl\nBBkkkkllll",
    "colors": {"a":"#AED293","b":"#8AAC71","c":"#6C9351","d":"#7F5D3D","e":"#62462C","f":"#395637","g":"#2C432A","h":"#DAC3FF","i":"#588054","j":"#4A6C47","k":"#BDF3A3","l":"#AADB93","A":"#BCEBFF","B":"#DFFFD0"}
  },
  "intro.sun": {
    "name": "Sun",
    "content": "AaaA\naaba\nBaBa\nAbaA",
    "colors": {"a":"#f59e0b","b":"#fbbf24","A":"#06b6d4","B":"#fcd34d"}
  },
  "intro.simple": {
    "name": "Demo",
    "content": "xxx",
    "colors": {"x":"#000"}
  },
  "intro.house": {
    "name": "House",
    "content": "ZZZbcZZZ\nZZabcdZZ\nZaabcddZ\naabbccdd\nZeefghiZ\nZeafghiZ\nZebffiiZ\njjkkklll",
    "colors": {"a":"#DF4848","b":"#C73A3A","c":"#A63636","d":"#7B2828","e":"#E4C8A7","f":"#CAAE8E","g":"#E7FFFE","h":"#D6F4F3","i":"#AB9070","j":"#DFFFD0","k":"#BDF3A3","l":"#AADB93","Z":"#BCEBFF"}
  },
  "intro.farm": {
    "name": "Farm",
    "content": "aaaaaaaaaaaa\naaaaaaaaaaaa\nABBABBABBABB\nAaaAaaAaaAaa\nCCDCCDCCDCCD\nbccbccbccbcc\nEdFEdFEFdEFd\nEddEddEddEdd\nbccbccbccbcc\nEFdEFdEdFEdF\nEddEddEddEdd\nbccbccbccbcc",
    "colors": {"a":"#BCEBFF","b":"#BF956E","c":"#62462C","d":"#9F7E60","A":"#5F4025","B":"#3C2817","C":"#AED293","D":"#8AAC71","E":"#7F5D3D","F":"#5E8C3E"}
  }
} as const;

export type NonogramKey = keyof typeof NonogramSources;

export const getNonogramTileMap = (srcKey: NonogramKey) => {
  const rawTileMap = NonogramSources[srcKey].content;
  return rawTileMap
    .split("\n")
    .map((line) => line.split("").map((tile) => tile.toUpperCase() !== tile));
};

export const getNonogramColorMap = (srcKey: NonogramKey) => {
  const rawTileMap = NonogramSources[srcKey].content;
  const colors = NonogramSources[srcKey].colors;
  return rawTileMap
    .split("\n")
    .map((line) => line.split("").map((tile) => colors[tile as keyof typeof colors] ?? '#fff'));
};
